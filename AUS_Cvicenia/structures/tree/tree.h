...
namespace structures
{
	...
	{
		//Done 07: TreeNode deepCopy
		TreeNode<T>* treeNode = this->shallowCopy();
		for (int i = 0, processedSons = 0; processedSons < this->degree(); i++) // for s viac premennymi
		{
			TreeNode<T>* son = this->getSon(i);
			if (son != nullptr)
			{
				treeNode->replaceSon(son->deepCopy(), i);
				processedSons++;
			}
		}
		return treeNode;
	...
	{
		//Done 07: TreeNode getBrother
		if (this->isRoot())
		{
			throw std::logic_error("TreeNode<T>::getBrother: Node is root and has no parent");
		}
	...
	{
		//Done 07: TreeNode sizeOfSubtree
		size_t size = 1;
		for (int i = 0, processedSons = 0; processedSons < this->degree(); i++) // for s viac premennymi
		{
			TreeNode<T>* son = this->getSon(i);
	...
	inline size_t Tree<T>::size()
	{
		...
		return this->root_->sizeOfSubtree();
	}
	...
	{
		//Done 07: Tree equalsTree
		//foreach nahradim iteratorom
		//cez iteratory je to O(2N) - 2x musim prehladat cely strom

		if (other == nullptr || this->size() != other->size()) //this->size() ma O(N)
		{
			return false;
		}

		Iterator<T>* currThis = this->getBeginIterator();
		Iterator<T>* endThis = this->getEndIterator();

		Iterator<T>* currOther = other->getBeginIterator();

		//iteratory vytvaraju new takze musim premazat pred kazdym return
		bool areEqual = true;

		while (*currThis != *endThis)
		{
			T itemThis = *(*currThis);
			T itemOther = *(*currOther);
	...
	{
		//Done 07: Tree clear
		delete this->root_; //destruktor v node musi vymazat podstrom
		this->root_ = nullptr;
	}
	...
	inline Tree<T>::TreeIterator::~TreeIterator()
	{
		//Done 07: Tree<T>::TreeIterator ~TreeIterator
		delete this->path_;
		this->path_ = nullptr;
	}
	...
	{
		//Done 07: Tree<T>::TreeIterator operator=
		if (this != &other)
		{
	...
	{
		//Done 07: Tree<T>::PreOrderTreeIterator populatePath
	...
	{
		//Done 07: Tree<T>::PostOrderTreeIterator populatePath
		if (current == nullptr)
		{
			return;
		}
	...
	{
		//Done 07: Tree<T>::LevelOrderTreeIterator populatePath
		if (current == nullptr)
		{
			return;
		}
		Queue<TreeNode<T>*>* processQueue = new ExplicitQueue<T>();
