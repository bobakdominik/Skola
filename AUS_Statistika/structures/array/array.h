...
namespace structures
{
	...
	{
		//Done 02: Array::assign
		if (this != &other)
		{
			Array<T>& otherArray = dynamic_cast<Array<T>&>(other); //dynamic cast z predka na potomka
	...
	void Array<T>::copy(Array<T>& src, int srcStartIndex, Array<T>& dest, int destStartIndex, int length)
	{
		...
		if (length > 0)
		{
			if (srcStartIndex < 0 || srcStartIndex + length - 1 >= static_cast<int>(src.size_))
			{
				throw std::out_of_range("Array<T>::copy: Invalid index in src vector");
			}
			if (destStartIndex < 0 || destStartIndex + length - 1 >= static_cast<int>(dest.size_))
			{
				throw std::out_of_range("Array<T>::copy: Invalid index in dest vector");
			}
